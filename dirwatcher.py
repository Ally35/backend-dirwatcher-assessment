...
def detect_added_files(files_dict, only_files, file_ext):
    """
    Iterates through file lists
    Checks for new files that match extension
    If this is the case, files are added to files_dict
    """
...
def detect_removed_files(files_dict, only_files):
    ...
    Compares to files in dictionary
    If dictionary item is not in files list, remove file
    """
    files_to_remove = []
...
def read_file(file_path, line_num, text, files_dict, file):
    ...
    Look for magic text within file
    If it finds it, log the line number and file where found
    """
    current_line = 1
...
def watch_directory(files_dict, watch_dir, file_ext, search_text):
    """Gets ran baised on polling interval.
    Builds list of current files
    Passes them along to add and delete detections
    Then calls read files to search for magic text"""
    try:
...
def calculate_run_time(start_time, end_time):
    ...
    and calculates time between the two.
    Returns string of days, hours, minutes, seconds"""
